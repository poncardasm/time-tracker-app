...
// Theme elements (queried in init)
let themeToggle;
let iconSun;
let iconMoon;
...
function init() {
  ...
  if (themeToggle) {
    ...
    console.error("Theme toggle button not found in initialization");
  }
}
...
function updatePomodoroStatusDisplay() {
  if (pomodoroState.phase === "work") {
    ...
    pomodoroPhaseText.className =
      "inline-block px-3 py-1 bg-indigo-100 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200 rounded-full text-xs font-bold uppercase tracking-wider";
  } else {
    pomodoroPhaseText.textContent = "Break Phase";
    pomodoroPhaseText.className =
      "inline-block px-3 py-1 bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200 rounded-full text-xs font-bold uppercase tracking-wider";
  }
}
...
function loadHistory() {
  const tasks = getTasks();
  totalTasksCount.textContent = `${tasks.length} task${
    tasks.length !== 1 ? "s" : ""
  }`;
  ...
  tasks.forEach((task, index) => {
    ...
    const startTimeStr = new Date(task.startTime).toLocaleTimeString(
      undefined,
      {
        hour: "2-digit",
        minute: "2-digit",
    ...
    row.innerHTML = `
            <td class="px-6 py-4">
                <input type="checkbox" class="task-checkbox rounded border-gray-300 text-cyan-600 focus:ring-cyan-500 cursor-pointer" data-index="${index}">
            </td>
            <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100 font-medium">
                <div class="flex items-center justify-between group">
                    <span>${escapeHtml(task.taskName)}</span>
                    <button class="text-gray-400 hover:text-cyan-600 dark:hover:text-cyan-400 opacity-0 group-hover:opacity-100 transition-opacity p-1 edit-task-btn" data-index="${index}">
            ...
            <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100 font-mono text-right">${formatTime(
              task.durationMs
            )}</td>
        `;
...
function showDeleteModal() {
  ...
  deleteModalText.textContent = `Are you sure you want to delete ${
    checkboxes.length
  } task${checkboxes.length !== 1 ? "s" : ""}? This action cannot be undone.`;
